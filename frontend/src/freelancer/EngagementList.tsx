...
export const EngagementList: React.FC = () => {
  ...
  const fetchEngagements = async () => {
    try {
      setLoading(true);
      const response = await api.get('/freelancers/me/engagements');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-gray-600">Manage your active projects and milestones</p>
      </div>
      ...
      {engagements.length === 0 ? (
        ...
        <div className="grid gap-4">
          {engagements.map((engagement) => (
            <Link
              key={engagement.id}
              to={`/freelancer/engagements/${engagement.id}`}
